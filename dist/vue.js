...
(function (global, factory) {
  ...
  function initEvents (vm) {
    ...
    var listeners = vm.$options._parentListeners; // 获取父组件在子组件标签中用v-on监听的事件
    if (listeners) {
      updateComponentListeners(vm, listeners); // 在子组件vm身上，注册、更新父组件对子组件vm监听的事件
    }
  }
  ...
  function initLifecycle (vm) {
    ...
    var parent = options.parent; // undefined
    // 根vue不走这个if
  ...
  function callHook (vm, hook) {
    ...
    pushTarget();
    var handlers = vm.$options[hook]; // 拿到options中的生命周期函数，如果有mixin，这里拿到的可能有多个生命周期函数
    var info = hook + " hook";
    if (handlers) {
  ...
  var Watcher = function Watcher (
    ...
    cb, // noop(空函数)
    options, // {before: function()},触发beforeUptate钩子函数。只有挂载过了才会执行，还未挂载时不会执行。
